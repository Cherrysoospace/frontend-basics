export default class Alquiler {
 ...
 static async init() {
  ...
  try {
    ...
    // crear las opciones para el select de instalaciones
  ...
  static #deleteRowClick = async (e, cell) => {
    ...
    Alquiler.#modal = new Modal({
      ...
      title: `<h5>Eliminación de Alquiler</h5>`,
      content: `<span class="text-back dark:text-gray-300">
                  Confirme la eliminación de:<br>
                  ${rowData.id} – ${rowData.socio.nombre}<br>
                  ${DateTime.fromISO(rowData.fechaHoraInicio).toFormat(window.formatDateTime.outputFormat)}<br>
                  Valor: $${rowData.valorAlquiler.toLocaleString()}<br>
                </span>`,
      buttons: [
        { caption: deleteButton, classes: 'btn btn-primary me-2', action: () => Alquiler.#delete(cell) },
        { caption: cancelButton, classes: 'btn btn-secondary', action: () => Alquiler.#modal.remove() },
      ],
    })
    Alquiler.#modal.show()
  ...
  const data = {
    ...
    fechaHoraInicio: document.querySelector(`#${Alquiler.#modal.id} #fechaHoraInicio`).value,
    fechaHoraFin: document.querySelector(`#${Alquiler.#modal.id} #fechaHoraFin`).value,
